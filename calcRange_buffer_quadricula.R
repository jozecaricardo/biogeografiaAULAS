# Range baseado num buffer
CalcRangeBuffer_q <- function(x, buffer.width = NULL, shape_file, resol, mean_dist = FALSE) {
  # projection
  ...
  dir.create('out_buffers/') # temporary folder
  ...
  for(j in unique(rownames(tempo))){
    ...
    write.shapefile(tempo_shape, paste0(c('out_buffers/pointshape_', j), collapse = ''))
    resul1_shape <- vect(paste0(c('out_buffers/pointshape_', j, '.shp'),
                                collapse = ''), crs = "+proj=longlat +datum=WGS84")
    # resul1_shape <- readShapeSpatial('tempshape1_out_buffers.shp') # shapefile
    ...
    ##### BUFFERS #####
    if(mean_dist == TRUE){
      mean_dista <- mean(terra::distance(x))
    ...
    #      alpha = 0.5)
    writeRaster(lista_r[[conta]], paste0(c("out_buffers/presence_BUFF_q", resol, j, ".tif"), 
  ...
  write.table(x = coor.l, file = 'out_buffers/pres_abs_BUFF_quad.txt', sep = '\t')
  
  ##################
  print(paste0(conta + 3, ') Loading the raster files and adding the presences...'))
  layers_MPC <- rast(list.files('out_buffers/', pattern = "\\.tif$", full.names = T))
